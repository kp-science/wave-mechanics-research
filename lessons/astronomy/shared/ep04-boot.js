...
(function(global){
  ...
  const Boot = {
    init(opts={}) {
      if (global.KPDB && global.KPDB.init) global.KPDB.init({ subject: 'astronomy', unit: 'ep04' });
  ...
  const Gate = {
    ...
    wireButton(currentPageId, buttonEl, nextHref, opts) {
      ...
      this._cfg = (global.PaceResolver && global.PaceResolver.get({ subject: 'astronomy', ep: 'ep04' })) || { enabled: false };
      this._update();
