/* ===== COSMOS LOG · EP05 Page Bootstrap ===== */
/* Boot · Chain · Gate · Submit · Transition · pattern เดียวกับ EP04        */

(function(global){
  const Boot = {
    init(opts={}) {
      if (global.KPDB && global.KPDB.init) global.KPDB.init({ subject: 'astronomy', unit: 'ep05' });
    ...
    /* ============ Quest HUD · HELION CHIPS + VOID ETA ============ */
    _injectQuestHud(pageId) {
      const hudPages = ['p05a','p05b','p05c','p06-bridge','p06a','p06b','p06c','p07-bridge','p07a','p07b','p07c','p08-bridge','p08a','p08b','p08c','p09-bridge','p09a','p09b','p09c','p10-bridge','p10a','p10b','p10c','p11','p12','p13','p14','p15','p16','p17','p18','p19','p20','p21','p22','p23','p24','p25'];
      ...
      if (!document.getElementById('questHudStyle')) {
        ...
        s.textContent = `
          #questHud { position:fixed; top:48px; left:50%; transform:translateX(-50%); z-index:50;
            ...
            background:rgba(8,4,12,0.95); border:1px solid rgba(255,138,61,0.5);
      ...
      const COLORS = { h1:'#ff5c7a', h2:'#ffcb6b', h3:'#ff8a3d', h4:'#b980ff', h5:'#ff5c7a', h6:'#7effb2' };
    ...
    _injectFooter(pageId) {
      ...
      foot.id = 'epFooter';
      foot.style.cssText = 'position:fixed;bottom:0;left:0;right:0;z-index:40;padding:8px 14px;background:rgba(5,6,15,0.92);border-top:1px solid rgba(255,138,61,0.18);display:flex;justify-content:space-between;align-items:center;gap:8px;backdrop-filter:blur(10px);';
      foot.innerHTML = `
        <a href="${prev ? prev.file : '#'}" style="color:${prev?'#ffcb6b':'#444'};text-decoration:none;font-size:12px;pointer-events:${prev?'auto':'none'};">${prev?'← '+prev.id+' '+prev.title:''}</a>
        <div style="font-family:Orbitron,monospace;font-size:10px;color:#6a7394;letter-spacing:0.15em;">EP05 · ${pageId||''} · ${idx>=0?(idx+1)+'/'+pages.length:''}</div>
        <a href="${next ? next.file : '#'}" style="color:${next?'#ff8a3d':'#444'};text-decoration:none;font-size:12px;pointer-events:${next?'auto':'none'};">${next?next.title+' '+next.id+' →':''}</a>
      `;
  ...
  global.EP05Boot = Boot;

  /* ============ Chain ============ */
  const CHAIN_KEY = 'cosmosLog_ep05_chain';
  const Chain = {
  ...
  /* ============ QuestState · HELION inventory + VOID ETA ============ */
  const QuestState = {
    helions() {
    ...
    voidEta() {
      ...
      return Number(cfg.voidEtaStartHr || 18);
    },
    decreaseEta(hours) {
    ...
    _refreshHud() {
      ...
      // Update chips · light up newly-collected ones with pop animation
    ...
    _burst(helionId) {
      ...
      if (!document.getElementById('helionKeyPopupStyle')) {
        ...
        s.textContent = `
          @keyframes hkBackdropIn { from{opacity:0;backdrop-filter:blur(0);} to{opacity:1;backdrop-filter:blur(6px);} }
          ...
          @keyframes hkCardOut { to{opacity:0;transform:scale(0.9) translateY(-10px);} }
          ...
          @keyframes hkSparkOut { 0%{opacity:1;transform:translate(-50%,-50%) scale(0);} 100%{opacity:0;transform:translate(calc(-50% + var(--sx)), calc(-50% + var(--sy))) scale(1.5);} }
          .hk-backdrop { position:fixed; inset:0; z-index:600; display:flex; align-items:center; justify-content:center;
            background:radial-gradient(ellipse at center, rgba(255,138,61,0.35), rgba(0,0,0,0.6) 70%);
            animation:hkBackdropIn .35s ease-out forwards; }
          ...
          .hk-card { position:relative; padding:28px 32px 22px; border-radius:22px; max-width:380px; width:88%;
            background:linear-gradient(160deg, rgba(40,16,8,0.98), rgba(8,4,12,0.98));
          ...
          .hk-spark { position:absolute; left:50%; top:50%; width:8px; height:8px; border-radius:50%;
            background:var(--hk-color); box-shadow:0 0 8px var(--hk-color);
            animation:hkSparkOut 1.2s ease-out forwards; pointer-events:none; }
          ...
          .hk-power { margin-top:12px; padding:10px 12px; border-radius:10px;
            background:linear-gradient(180deg, rgba(255,215,0,0.18), rgba(255,138,61,0.08));
      ...
      // POWER preview · บอกว่ากุญแจนี้จะแลกเป็นพลังอะไรใน p24 → p25
      const POWER_MAP = {
        h1: { icon:'⚛️', name:'NUCLEAR BURST',  desc:'ตอบผิด 1 ข้อ → ได้คะแนนครึ่ง' },
        h2: { icon:'💡', name:'INSIGHT BEAM',   desc:'กดเผยคำตอบที่ถูก 1 ข้อ' },
        h3: { icon:'⏱️', name:'TIME BUBBLE',    desc:'กดเพิ่ม CME-ETA +6 ชม.' },
        h4: { icon:'🛡️', name:'FIELD SHIELD',   desc:'auto · กัน CME pulse 1 ครั้ง' },
        h5: { icon:'📡', name:'EARLY WARNING',  desc:'auto · เตือน 2 วิก่อน pulse' },
        h6: { icon:'🌟', name:'PRISMATIC KEY',  desc:'ปลด ending A+ เมื่อชนะ boss' },
      };
      ...
      // 12 sparks (decorative · burst out from center)
      const sparksHtml = Array.from({length: 12}).map((_, i) => {
        const ang = (i / 12) * Math.PI * 2;
        const dist = 90 + Math.random() * 30;
        const dx = Math.cos(ang) * dist, dy = Math.sin(ang) * dist;
        const delay = (Math.random() * 0.4).toFixed(2);
        return `<div class="hk-spark" style="--sx:${dx.toFixed(0)}px;--sy:${dy.toFixed(0)}px;animation-delay:${delay}s;"></div>`;
      }).join('');

      const hintText = isLast
        ? `<b>🎉 ครบ 6/6 แล้ว!</b> · ไปที่ <b>p24 POWER FORGE</b> เปิดกล่องด้วยรหัส 6 หลัก แล้วแลกเป็นพลังสำหรับสู้ VOID ที่ <b>p25 BOSS</b>`
        : `เก็บกุญแจไว้ก่อน · ยังเหลืออีก <b>${remaining} ชิ้น</b> · ครบ 6 แล้วเอาไปเปิดกล่องที่ <b>p24 POWER FORGE</b>`;

      backdrop.innerHTML = `
        <div class="hk-card" style="--hk-color:${color};">
          <div class="hk-banner">${isLast ? '🏆 FINAL KEY · 6/6' : '🔑 KEY UNLOCKED'}</div>
          <div class="hk-title">คุณได้รับกุญแจพลังงาน</div>
          <div class="hk-icon-wrap">
            ${sparksHtml}
            <div class="hk-ring"></div>
            <div class="hk-icon">${hel ? hel.icon : '🔆'}</div>
          </div>
          <div class="hk-name">${hel ? hel.name : 'HELION CORE'}</div>
  ...
  /* ============ Phase3 · mini-quiz helper (3-phase template) ============ */
  const Phase3 = {
    /**
     * Phase3.mount(opts)
     *   container: DOM element to render quiz
     *   question: string
     *   choices: [{txt, correct, hint}]
     *   onCorrect: () => void  (called when user picks correct)
     *   helionId: optional · grant on correct
     */
    mount(opts) {
      ...
      opts.choices.forEach((choice, i) => {
        ...
        b.onclick = () => {
          ...
          if (choice.correct) {
            ...
            b.style.opacity = '0.6';
            b.disabled = true;
            fb.innerHTML = '<span style="color:#ff5c7a;">✗ ' + (choice.hint || 'ลองใหม่ · ดูที่กิจกรรมข้างบนอีกครั้ง') + '</span>';
  ...
  const Gate = {
    ...
    wireButton(currentPageId, buttonEl, nextHref, opts) {
      ...
      this._cfg = (global.PaceResolver && global.PaceResolver.get({ subject: 'astronomy', ep: 'ep05' })) || { enabled: false };
      this._update();
  ...
  const Submit = {
    record(page, payload) {
      const key = 'cosmosLog_ep05_submit_' + page;
    ...
    load(page) {
      try { return JSON.parse(localStorage.getItem('cosmosLog_ep05_submit_' + page)) || {}; } catch { return {}; }
    },
    wirePair(opts) {
  ...
  /* ============ Transition · cinematic (with EP05 solar variants) ============ */
  const Transition = {
    play(tag, callback) {
      ...
      switch(tag) {
        case 'WARP_APPROACH':
          html = `<div style="position:absolute;inset:0;background:radial-gradient(circle at 50% 50%, rgba(255,203,107,0.45), transparent 70%); animation:warpApp 0.9s;"></div>`;
          break;
        case 'CME_BURST':
          html = `<div style="position:absolute;inset:0;background:radial-gradient(circle, #fff 0%, #ffcb6b 18%, #ff5c7a 45%, transparent 75%); animation:cmeBurst 1.2s;"></div>`;
          dur = 1200;
          break;
        case 'SOLAR_FLARE':
          html = `<div style="position:absolute;inset:0;background:linear-gradient(120deg, transparent 30%, #ffcb6b 50%, transparent 70%); animation:flareSweep 1s;"></div>`;
          dur = 1000;
          break;
        case 'AURORA':
        ...
        case 'VOID_REVEAL':
          ...
          break;
        case 'ECLIPSE':
          html = `<div style="position:absolute;inset:0;background:radial-gradient(circle at 50% 50%, transparent 18%, #000 22%, #000 100%); animation:eclipse 1.4s;"></div>`;
          dur = 1400;
          break;
        default:
      ...
      el.innerHTML = html;
      if (!document.getElementById('trnStyleEp05')) {
        ...
        s.textContent = `
          ...
          @keyframes flareSweep { 0%{opacity:0;transform:translateX(-50%);} 50%{opacity:1;transform:translateX(0);} 100%{opacity:0;transform:translateX(50%);} }
          ...
          @keyframes eclipse { 0%{opacity:0;transform:scale(0.3);} 50%{opacity:1;transform:scale(1);} 100%{opacity:0;transform:scale(1.5);} }
        `;
        document.head.appendChild(s);
      }
